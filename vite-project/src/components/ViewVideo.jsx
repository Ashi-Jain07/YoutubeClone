...
function ViewVideo() {
    ...
    async function handleLikes(findVideo) {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5500/video/${id}`, {
                method: "PATCH",
                headers: {
    ...
    async function handleEditComment(id, index) {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5500/editcomment/${id}`, {
                method: "PATCH",
                headers: {
                    "Accept": "application/json",
                    "Content-Type": "application/json",
    ...
    async function handledeleteComment(id, index) {
        ...
        try {
            const response = await fetch(`http://localhost:5500/deletecomment/${id}`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    authorization: `JWT ${token}`
            ...
            <div className="md2:flex md2:flex-row md2:justify-evenly md2:ml-5 md:ml-28 sm:ml-16 lsm:ml-8 m-1 lg+:ml-10 mt-16">
                ...
                <div className="mt-10">
                    ...
                    <div className="flex justify-between">
                        ...
                        <div className="flex">
                            <button className="h-8 p-1 m-1 bg-slate-300 text-center flex">
                                <img src={isLiked ? "/after-like.png" : "/before-like.png"} width="15px" height="15px" className="mt-1" onClick={() => handleLikes(findVideo)} />
                                {likes}
                            </button>
                            <button className="h-8 p-1 m-1 bg-slate-300 text-center flex">
                                <img src={isDislike ? "/after-dislike.png" : "/before-dislike.png"} width="15px" height="15px" className="mt-1" onClick={() => handleDislikes(findVideo._id)} />
                                {dislikes}
                            </button>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <button onClick={() => handleAddComment(findVideo._id)} className="font-semibold ml-2 text-lg">Post</button>
                        <div className="mt-5 bg-slate-200 p-2 w-9/12">
                            {/* Map on the comments */}
                            {findVideo.comments.map((data, index) => (
                                <div key={index} className="border border-black shadow-sm p-1 bg-white shadow-black w-11/12 mt-5">
                                        ...
                                        <div>
                                            ...
                                            {editIndex === index ? (
                                                ...
                                                <button onClick={() => {
                                                    ...
                                                    <img src="/edit-button.png" alt="Edit" width="20px" height="20px" />
                                                </button>
                                            )}
                                            <button onClick={() => handledeleteComment(findVideo._id, index)}><img src="/delete.png" alt="delete" width="20px" height="20px" /></button>
                                        </div>
                                    </div>
                                </div>
                            ))}
