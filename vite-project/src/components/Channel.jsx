...
function Channel() {
    ...
    async function handleChannelDelete(id) {
        try {
            const response = await fetch(`http://localhost:5500/deletechannel/${id}`, {
                method: "DELETE",
                headers: {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 place-items-center m-5">
                    {
                        details.map(item => (
                            ...
                            <div key={item._id} className="mb-5">
                                ...
                                <button onClick={() => handleChannelDelete(item._id)}><img src="/delete.png" width="20px" height="20px" /></button>
                            </div>
